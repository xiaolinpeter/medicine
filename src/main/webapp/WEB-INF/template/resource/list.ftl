							...
							<div class="form-horizontal">
								...
								<div id="menuInfoDiv">
									<div class="form-group" id="menuIconDiv">
										...
										<div class="col-sm-10">
											<button id="menuIcon" class="btn btn-default"
												data-iconset="fontawesome" data-icon="fa-bars"
									...
									<div class="form-group" id="parentDiv">
										...
										<div class="col-sm-6">
											<div class="btn-group">
												<button id="parentButtonTip" type="button"
												...
												<ul class="dropdown-menu" role="menu">
													...
													<c:forEach items="${menuList }" var="menu"
														...
														<c:if test="${empty menu.parentId and empty menu.url}">
															<li dir="${menu.id }"><a href="javascript:void(0)"
																style="text-decoration: none; color: #000"><i
																	class="${menu.icon }" style="margin-right: 0.5rem;"></i>
																	${menu.name }</a></li>
														</c:if>
													</c:forEach>
													-->
														<#list resourceList as resource>
															<#if (resource.parentId?? && resource.url?? && resource.isMenu == 1)>
																<li dir="${resource.id }"><a href="javascript:void(0)"
																	style="text-decoration: none; color: #000"><i
																		class="${resource.icon!}" style="margin-right: 0.5rem;"></i>
																		${resource.name}</a></li>
															</#if>
													   </#list>
									...
									<div class="form-group">
										...
										<div class="col-sm-10">
											<input id="menuId" type="hidden"> <input
												id="menuName" type="text" class="form-control"
												placeholder="请输入菜单名称">
										</div>
									</div>
									<div class="form-group">
									...
									<div class="form-group">
										...
										<div class="col-sm-10">
											<input type="text" class="form-control" id="menuOrder"
												placeholder="请输入菜单顺序"
												onkeyup="if(/\D/.test(this.value)){ShowFailure('只能是数字');this.value='';}else{if(this.value>250){ShowFailure('不能大于200');this.value='';}}">
										</div>
									</div>
								</div>
							</div>
							...
							<div class="form-group" id="menuDiv">
								...
								<div class="col-sm-8">
									...
									<div class="col-lg-8" id="menuEditDiv" style="display: none">
										<div class="form-group">
											<label>菜单名称</label> <input id="editMenuName"
												class="form-control" type="text" placeholder="请输入菜单名称">
										</div>
										<div class="form-group">
											<label>菜单图标</label>
											<button id="editMenuIcon" class="btn btn-default"
												data-iconset="fontawesome" data-icon="fa-bars"
												role="iconpicker"></button>
										</div>
										<div class="form-group">
											<label>父菜单</label> <i class="fa fa-square"></i> <span
												id="editMenuParent"></span>
										</div>

										<div class="form-group">
											<label>菜单地址</label> <input id="editMenuUrl"
												class="form-control" type="text" placeholder="请输入菜单地址">
										</div>
										<div class="form-group">
											<label>菜单顺序</label> <input id="editMenuOrder"
												class="form-control" type="text" placeholder="请输入菜单顺序"
												onkeyup="if(/\D/.test(this.value)){ShowFailure('只能是数字');this.value='';}else{if(this.value>250){ShowFailure('不能大于200');this.value='';}}">
										</div>
										  <div class="form-group">
...
<script type="text/javascript"
	src="${base}/resources/plugins/bootstrap-plugin/bootstrap-iconpicker/js/iconset/iconset-fontawesome-4.2.0.min.js"></script>
<!-- Bootstrap-Iconpicker -->
<script type="text/javascript"
	src="${base}/resources/plugins/bootstrap-plugin/bootstrap-iconpicker/js/bootstrap-iconpicker.min.js"></script>
<!-- easyform -->
<script src="${base}/resources/plugins/easyform/easyform.js"></script>
<script type="text/javascript"
...
<script type="text/javascript"
	src="${base}/resources/plugins/ztree/js/jquery.ztree.exedit.js"></script>
<script>
    ...
    function onRemove(e,treeId,treeNode){  
		...
		function deleteMenu(id){
			$.ajax({
				url :'${base}/admin/resource/delete/'+id,
				type : 'POST',
    ...
    function clickNode(e,treeId,treeNode){  
		...
		// addOrUpdateMenu(id,name,rank,icon,url,parentId)
		var editMenuId;
		var editMenuName;
		var editMenuOrder;
		var editMenuIcon;
		...
		function showEditMenu(treeNode){
			...
			$('#loadingToast').show();
			$.ajax({
				url : '${base}/admin/resource/detail/'+treeNode.id,
				type : 'get',
				async : false,
				success : function(result) {
					result = $.parseJSON(result);
			...
			$("#editMenuOrder").val(editMenu.rank);
			$("input[name='isMenu'][value='"+editMenu.isMenu+"']").prop("checked","checked");
			$('#loadingToast').hide();
		}
		
		$("#btnSubmitEdit").click(function(){
			var name=$("#editMenuName").val().trim();
			var rank=$("#editMenuOrder").val().trim();
			...
			addOrUpdateMenu(editMenuId,name,rank,editMenuIcon,url,isMenu,editMenuParentId)
		})
		...
		$("#btnSubmit").click(function(){
			...
			if(isNaN(menuStyle)){
				...
				if(menuStyle==3){// 父菜单下的子菜单
					...
					addOrUpdateMenu(menuId,menuName,menuOrder,menuIcon,menuUrl,isMenu,'');
				}
		...
		function addOrUpdateMenu(id,name,rank,icon,url,isMenu,parentId){
			$.ajax({
				url : '${base}/admin/resource/add',
				type : 'post',
				data : {
					'id':id,
					'name' : name,
